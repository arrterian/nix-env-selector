...
import {
  node, of, FutureInstance, reject,
} from "fluture";
import {
  fold, fromNullable, Option, mapNullable, none, some,
...
import {
  Label, Notification, Command, NOT_MODIFIED_ENV,
} from "./constants";
...
export const updateEditorConfig = (
  ...
  node<Error, boolean>(done =>
    config
      ...
      .then(() => done(null, true), err => done(err)),
  );
...
export const activateOrShowDialog = (workspaceRoot: string, nixAttr: Option<string>) =>
  fold<string, FutureInstance<Error, Option<boolean>>>(
    () => getNixConfigList(workspaceRoot)
